...
debug_ui.show_window = function(player)
    ...
    if not root.tvc_debug_ui then
        ...
        table.add({type = "label", caption = "Monetary"})
        ...
        table.add({type = "label", caption = "Subs"})
        ...
        table.add({type = "label", caption = "Misc"})
...
debug_ui.events = {
    [defines.events.on_console_command] = function(event)
        if event.player_index and event.command == "tvc_debug" then
